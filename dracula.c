...
{	
	...
	for (int i = 0; i < (int) numLocations; i++) {
		for (int z = 0; z < (int) strlen(reachableLocHunters); z++) {
			if (strcmp(yo[i], reachableLocHunters[z]) == 0) break;
			if (z == (int) strlen(reachableLocHunters) - 1) {
		...
		if (i == (int) numLocations - 1) {
			for (int k = 0; k < 4; k++) {
				location_t g = dv_get_location(dv, k);
				if ((strcmp(location_get_abbrev (g), "KL") == 0) || 
				strcmp(location_get_abbrev (g), "GA") == 0) {
				break;
				if (i == 3) { 
